...
export default function EduSentinelDashboard() {
  ...
  // Live dynamic state from FastAPI/Supabase backend
  const [students, setStudents] = useState<Student[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  // Advisory Chatbot State
  ...
  // Fetch live student data from FastAPI when search query changes
  useEffect(() => {
  ...
  // KPI Calculations dynamically derived from live database records
  ...
  // Filter logic for anomaly category dropdown
  const filteredStudents = students.filter((s) => {
    return selectedFilter === "ALL" || s.anomaly_category === selectedFilter;
  });

  // Handle Chatbot Queries (Connected to backend or fallback dynamic logic)
  const handleSendMessage = async (): Promise<void> => {
    ...
    // Fallback client-side dynamic parsing if backend endpoint is bypassed
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
              <input
                type="text"
                placeholder="Search student name or 6-digit ID..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full md:w-80 px-3 py-2 text-xs border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
              />
              <select
                value={selectedFilter}
                onChange={(e) => setSelectedFilter(e.target.value)}
                className="text-xs border border-slate-300 rounded-lg px-3 py-2 bg-slate-50 outline-none focus:ring-2 focus:ring-indigo-500"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredStudents.map((student) => (
                  <div
                    ...
                    className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3 hover:-translate-y-0.5 transition"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-sm text-slate-800">{student.full_name}</h3>
                        <p className="text-xs text-slate-500">
                          ID: {student.student_id} | {student.class_standing}
                        </p>
                      </div>
                      <span
                    ...
                    <button
                      ...
                      className="w-full text-center bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold py-1.5 rounded-lg border border-indigo-100 transition"
                    >
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            {selectedStudent ? (
              <>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4">
                  <div>
                    <span className="text-xs font-semibold text-indigo-600 uppercase tracking-wider">
                      USIU Anomaly Audit Log
                    </span>
                    <h2 className="text-xl font-bold text-slate-800">
                      {selectedStudent.full_name} (ID: {selectedStudent.student_id})
                    </h2>
                    <p className="text-xs text-slate-500">
                      Major: {selectedStudent.declared_major} | {selectedStudent.class_standing}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-red-50/50 p-5 rounded-xl border border-red-200 space-y-3">
                    <h3 className="font-bold text-xs text-red-800 uppercase tracking-wider">
                      ⚡ Triggered Risk Flags
                    </h3>
                    <ul className="text-xs space-y-2 text-slate-700">
                      {selectedStudent.anomalies?.map((a, i) => (
                        <li key={i} className="flex items-start gap-2">
                          <span className="text-red-500">⚠️</span> {a}
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <p className="text-slate-400 text-xs text-center py-8">
                Select a student from the Risk Dashboard to view full anomaly details.
              </p>
            )}
          </div>
        )}

        {/* TAB 3: AI ADVISORY BOT */}
        {activeTab === "chatbot" && (
          <div className="max-w-3xl mx-auto bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[500px]">
              ...
              {chatMessages.map((msg, i) => (
                <div
                  ...
                  className={`p-3 rounded-lg max-w-lg whitespace-pre-line ${
                    msg.sender === "user"
                      ? "bg-indigo-50 text-indigo-950 ml-auto font-medium"
            ...
            <div className="p-3 border-t border-slate-200 flex gap-2">
              <input
                type="text"
                placeholder="Ask e.g. What are prerequisites for APT2060 or student ID 664101?"
                value={chatInput}
                onChange={(e) => setChatInput(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSendMessage()}
                className="flex-1 border border-slate-300 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <button
